...
final class Breadcrumb {
	...
	public function getBreadcrumb() {
		...
		// Archive post type: team
		if ( is_archive() && get_post_type() == 'team' ) {
			$output .= $this->getCurrentLink( 'Team' );
		}
		...
		if ( is_singular( 'projects' ) ) {
			...
			if ( ! empty( $page ) ) {
				$output .= $this->getLink( 'Projects', get_permalink( $page->ID ) ).
					   	   $this->getSeparator().
					   	   $this->getCurrentLink( get_the_title() );
			}
		}
	...
	private function getHome() {
		$link = '<a class="ma-breadcrumb__link" href="'. home_url() .'">'. $this->home_title .'</a>';
	...
	private function getSeparator() {
		$separator = '<span class="ma-breadcrumb__separator">'. $this->separator .'</span>';
	...
	private function getCurrentLink( $title ) {
		$current_link = '';
		if ( ! empty( $title ) ) {
			$current_link = '<span class="ma-breadcrumb__current">'. esc_html( $title ) .'</span>';
		}
	...
	private function getLink( $title, $url ) {
		...
		if ( ! empty( $title ) && ! empty( $url ) ) {
			$link = '<a class="ma-breadcrumb__link" href="'. esc_url( $url ) .'">'. esc_html( $title ) .'</a>';
		}
		return $this->getWrapped( $link );
	}
